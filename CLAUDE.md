...
- **Add-ons cannot make a world object glow.** ESO modding is UI-only — no shader, material, or outline access. The only real sack highlight in the game is Zerith-Var's rapport perk. What *is* possible is drawing your own marker at a remembered position (`Create3DRenderSpace` / `Set3DRenderSpaceOrigin`), which is what `addons/MooseSackGlow/` does — and ZOS disables that 3D API inside dungeons and trials on purpose. Don't re-pitch "just make them glow"; the answer is a marker on the spot. Written up in `docs/notes/sack-glow-addon.md`.
