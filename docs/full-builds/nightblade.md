...
- **Above and Beyond** — flat +25% Critical Damage **and Healing** at all times (raises your crit cap by 30% too). The healing half multiplies every layered NB heal you run — Leeching, the spectral bow, Killer's Blade. Pure "don't die" glue that also parses.
...
*\*Dual daggers are the default — best damage and the up-close playstyle you run. Dagger enchants: main-hand **Poison**, off-hand **Absorb Stamina** (that off-hand Absorb Stamina is part of the answer to your documented stamina gap — see the sustain note below). The bow back bar exists only to lay Endless Hail + Barbed Trap and refresh Relentless Focus; you swap back to daggers immediately. You spend the fight in melee.*
...
- **Merciless Resolve** is the *magicka* morph of Grim Focus; **Relentless Focus** is the *stamina* morph (Physical damage bow) — you want Relentless on a stamblade. Both grant Minor Berserk while slotted.
- **Ulfsild's Contingency** needs Scribing (Gold Road), which you have. If you ever want that slot non-scribed, use **Refreshing Path** (morph of Path of Darkness, *Shadow*) — a heal-over-time path plus Major Expedition; more layered healing, slightly less damage.

### Situational swaps (with skill line sources)
- **Refreshing Path** — *Nightblade > Shadow* (morph of Path of Darkness) — stand-in-it heal-over-time + Major Expedition; the maximum-survivability swap for Ulfsild's or Barbed Trap
...
2. **Leeching Strikes** up (it's your sustain *and* a heal — never let it lapse)
...
Your documented DK stamina shortfall was diagnosed as **light-attack weaving gaps**, not skill costs. On this Nightblade the same weaving is doubly rewarded: **Leeching Strikes returns stamina on every weave**, and **Relentless Focus turns your light attacks into a healing burst**. So the discipline that was *costing* you on the DK is what *funds* this build. If stamina still dips, the cause is the same one — you're skipping light attacks between skills — not gear. The off-hand **Absorb Stamina** dagger enchant is the backstop.
...
**Potions:** Weapon Power potions (Blessed Thistle + Dragonthorn + Wormwood). Between Leeching Strikes and the Absorb Stamina enchant you should not need Tri-Restoration potions — if you do, it's a weaving problem, not a potion problem.
...
- **Leeching Strikes can come off** for a pure DoT or buff skill — the group heals you, and Relentless Focus + Killer's Blade still cover you. Keep it if the fight is chaotic.
- **Dark Shade / Shadowy Disguise stay** — Minor Maim and Minor Protection are still free personal mitigation, and Shadowy Disguise's +15% damage is a parse gain, not just a defensive.
- Barbed Trap stays (Minor Force is a group crit-damage buff if no one else brings it; drop it if someone does).
- Soul Harvest stays as the execute ult; it also applies a debuff that helps the group's burn.

**Do not guess a raid rotation from this page.** Point yourself at the live **Hyperioxes group Stamina Nightblade DPS build** and copy its bar and set list before a progression run — that's the authoritative source, and it changes between updates.
...
**What carries over:** Surprise Attack burst, Killer's Blade execute (deadly in PvP under 28%), Shadowy Disguise (the cloak *is* the class), Leeching Strikes sustain, Soul Harvest / Soul Tether burst ultimates.
...
| 7 | Eldritch Insight | buy max (20) | max magicka (Leeching feeds it) |
...
| 14 | **Reaving Blows** | buy (50), swap option | heals off direct damage — stacks with Pale Order *and* Leeching for absurd solo healing |
...
| 3 | **Rejuvenation** | **SLOT** (50) | recovery — kept slotted for *you specifically*: unlike your DK (Soul of Flame solved sustain), your history says keep a recovery star even though Leeching does the heavy lifting |
...
| 11 | Siphoning Spells | buy (50), swap option | extra magicka return if a fight out-drains Leeching |
